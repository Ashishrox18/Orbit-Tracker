...
export async function addWordManual(
  ...
  const inserted = await db
    ...
    .onConflictDoUpdate({
      ...
      set: { learnedOn: day, meaning: input.meaning, partOfSpeech: input.partOfSpeech, etymology: input.etymology, examples: input.examples },
    })
    .returning();
  ...
  // Add to review queue so it shows up in spaced repetition
